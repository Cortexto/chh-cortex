...
def build_full_application() -> Path:
    ...
    add_label_para(doc, "Trainee. ", "Aral Surmeli is a DrPH candidate advised by William (Bill) Weiss and the physician-founder of HERA Digital Health. He is first author of the current JMIR preprint and registered principal investigator for NCT05094518, which evaluated antenatal-care reminders among Syrian refugee women at two Istanbul migrant health centers.[10] Under Elnakib's supervision, Surmeli will lead operational data-provenance mapping, linkage-feasibility documentation, implementation protocol and fidelity assessment, and rapid-cycle learning. His earlier publications on HERA's refugee-health model and end-user acceptability provide additional population and implementation context.[8,9]")
